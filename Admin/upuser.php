...
{
	$cn=makeconnection();
	$s="update admin set password='" .$_POST["t2"] . "',typeofuser='". $_POST["s1"] ."' where username='" . $_POST["s2"] ."'";
	mysqli_query($cn,$s);
	mysqli_close($cn);
...
{
$cn=makeconnection();
$s="select * from admin where username='" .$_POST["s2"] ."'";
	$result=mysqli_query($cn,$s);
	$r=mysqli_num_rows($result);
	//echo $r;
...
<option value="Admin" <?php if(isset($_POST["show"])&& $usertype==1){ echo "selected "; } ?>>Admin</option>
<option value="General" <?php if(isset($_POST["show"])&& $usertype==0){ echo "selected "; } ?>>General</option>
</select></td></tr>
